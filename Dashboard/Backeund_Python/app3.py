...
def detect_anomaly(data_point):
    ...
    try:
        ...
        display_features_string = ", ".join(anomalous_features_list) if status == "Anomali" else "-"
        
        return {"detection_status": status, 
                "anomalous_features": display_features_string, 
        ...
        try:
            with db_connection.cursor() as cursor:
                for item in batch_data:
                    # 'fitur_anomali' sudah berupa string, tidak perlu json.dumps
                    fitur_anomali_str = item['fitur_anomali']
                    cursor.execute('''
...
async def main_async_worker():
    ...
    while True:
        try:
            ...
            print(f"[INFO] Status: {anomali_status}, Fitur Anomali: {fitur_anomali if fitur_anomali else 'N/A'}")

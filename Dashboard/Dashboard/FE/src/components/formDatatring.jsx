...
//           <h3 className="text-[18px] font-medium mb-2" style={{ userSelect: 'none' }}>Tanggal Pengukuran</h3>
//           <input
//             className="border-[#E5E5E5] border-2 text-center text-[32px] font-medium w-full h-[80px] mb-4 sm:mb-0"
//             type="date" // Menggunakan tipe "date" akan memunculkan kalender
//             value={data.timestamp || ""} // Gunakan || "" untuk nilai awal
...
const InputForm = ({ data, onChange, onSave }) => {
  const handleInputChange = (e, field) => {
    const value = e.target.value;
    // Untuk tipe 'date', nilainya sudah string, tidak perlu validasi isNaN
    if (field === 'timestamp') {
  ...
  return (
    <div className="card bg-white rounded-2xl px-6 py-8 my-6 mx-auto">
      <div className="card-body grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 mt-6">
        ...
        {/* Save Button */}
        <div className="flex justify-center items-center col-span-1 md:col-span-3 lg:col-span-3">
          <button
            className="bg-[#616161] hover:bg-[#383B4C] text-white border-2 rounded-lg text-center text-2xl font-medium w-full lg:w-[30%] h-[80px] mt-4 sm:mt-0"
            onClick={onSave}
          >
            Save & Predict
          </button>

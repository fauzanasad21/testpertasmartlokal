...
const DrynessPage = () => {
  ...
  const [selectedTitle, setSelectedTitle] = useState("Dryness Chart Data");
  ...
  const fetchTableData = async (page, rowsPerPage, period) => {
    let params = `type=dryness&period=${period}`;
  ...
  const handleDownloadAllData = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Statistics_Dryness_${selectedPeriodForTable}.csv`
      );
  ...
  const fetchCardData = async () => {
    try {
      const [
        ...
        fetch(
          `http://localhost:9921/api/statistics?period=now&type=dryness`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=day&type=dryness`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=month&type=dryness`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=year&type=dryness`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
      ...
      const drynessData = dataLimitBatas.find((item) => item.data === "dryness");
      //console.log('batas', drynessData)

      let batasAtas = drynessData?.upperlimit ?? 90;
      let batasBawah = drynessData?.bottomlimit ?? 20;
      ...
      //console.log('calibration down ', minCali)
      //console.log('batas bawah ', batasBawah);
      //console.log('batas atas ', batasAtas)

      //console.log('calibration up ', maxCali)
      setDataCali({
        CaliMin: minCali,
        CaliMax: maxCali,
      });
      ...
      setMinMaxAvgNow({
        minNow: dataNowResponse.min_dryness,
        maxNow: dataNowResponse.max_dryness,
        avgNow: dataNowResponse.avg_dryness,
        stddevNow: dataNowResponse.stddev_dryness,
        trendNow: dataNowResponse.trend_status,
        outOfLimitNow: dataNowResponse.out_of_limit_duration,

        minDaily: dataDaily.min_dryness,
        maxDaily: dataDaily.max_dryness,
        avgDaily: dataDaily.avg_dryness,
        stddevDaily: dataDaily.stddev_dryness,
        trendDaily: dataDaily.trend_status,
        outOfLimitDaily: dataDaily.out_of_limit_duration,

        minMonthly: dataMonthly.min_dryness,
        maxMonthly: dataMonthly.max_dryness,
        avgMonthly: dataMonthly.avg_dryness,
        stddevMonthly: dataMonthly.stddev_dryness,
        trendMonthly: dataMonthly.trend_status,
        outOfLimitMonthly: dataMonthly.out_of_limit_duration,

        minYearly: dataYearly.min_dryness,
        maxYearly: dataYearly.max_dryness,
        avgYearly: dataYearly.avg_dryness,
        stddevYearly: dataYearly.stddev_dryness,
  ...
  const handleCardClick = async (data) => {
    try {
      ...
      let params = `type=dryness&period=${selectedPeriodForApi ?? "daily"}`;
      ...
      if (data.rule === "min") {
        setSelectedTitle(`Minimum dryness`);
        setSelectedData(
      ...
      if (data.rule === "max") {
        setSelectedTitle(`Maximum dryness`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
      ...
      if (data.rule === "avg") {
        setSelectedTitle(`Average dryness`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
      ...
      if (data.rule === "stddev") {
        setSelectedTitle(`Standard Deviation dryness`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
  ...
  const fetchAllHistoricalData = async (startDate, endDate) => {
    const [dryness] = await Promise.all([
  ...
  return (
    <>
      <div className="bg-white rounded-lg p-4 mb-5 flex items-center justify-between mt-10 md:mt-20 lg:mt-5">
        <p className="text-2xl font-bold"> Statistics Dryness </p>
      </div>

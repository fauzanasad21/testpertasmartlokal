...
const PressurePage = () => {
  ...
  const [selectedTitle, setSelectedTitle] = useState("Pressure Chart Data");
  ...
  const fetchTableData = async (page, rowsPerPage, period) => {
    let params = `type=pressure&period=${period}`;
  ...
  const handleDownloadAllData = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Statistics_Pressure_${selectedPeriodForTable}.csv`
      );
  ...
  const fetchCardData = async () => {
    try {
      const [
        ...
        fetch(
          `http://localhost:9921/api/statistics?period=now&type=pressure`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=day&type=pressure`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=month&type=pressure`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=year&type=pressure`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
      ...
      const pressureData = dataLimitBatas.find(
        (item) => item.data === "pressure"
      );
      let batasAtas = pressureData ? pressureData.upperlimit : null;
      let batasBawah = pressureData ? pressureData.bottomlimit : null;
      ...
      setMinMaxAvgNow({
        minNow: dataNowResponse.min_pressure,
        maxNow: dataNowResponse.max_pressure,
        avgNow: dataNowResponse.avg_pressure,
        stddevNow: dataNowResponse.stddev_pressure,
        trendNow: dataNowResponse.trend_status,
        outOfLimitNow: dataNowResponse.out_of_limit_duration,

        minDaily: dataDaily.min_pressure,
        maxDaily: dataDaily.max_pressure,
        avgDaily: dataDaily.avg_pressure,
        stddevDaily: dataDaily.stddev_pressure,
        trendDaily: dataDaily.trend_status,
        outOfLimitDaily: dataDaily.out_of_limit_duration,

        minMonthly: dataMonthly.min_pressure,
        maxMonthly: dataMonthly.max_pressure,
        avgMonthly: dataMonthly.avg_pressure,
        stddevMonthly: dataMonthly.stddev_pressure,
        trendMonthly: dataMonthly.trend_status,
        outOfLimitMonthly: dataMonthly.out_of_limit_duration,

        minYearly: dataYearly.min_pressure,
        maxYearly: dataYearly.max_pressure,
        avgYearly: dataYearly.avg_pressure,
        stddevYearly: dataYearly.stddev_pressure,
  ...
  const handleCardClick = async (data) => {
    try {
      ...
      let params = `type=pressure&period=${selectedPeriodForApi ?? "daily"}`;
      ...
      if (data.rule === "min") {
        setSelectedTitle(`Minimum pressure`);
        setSelectedData(
      ...
      if (data.rule === "max") {
        setSelectedTitle(`Maximum pressure`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
      ...
      if (data.rule === "avg") {
        setSelectedTitle(`Average pressure`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
      ...
      if (data.rule === "stddev") {
        setSelectedTitle(`Standard Deviation pressure`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
  ...
  return (
    <>
      <div className="bg-white rounded-lg p-4 mb-5 flex items-center justify-between mt-10 md:mt-20 lg:mt-5">
        <p className="text-2xl font-bold"> Statistics Pressure </p>
      </div>

...
const FlowPage = () => {
  ...
  const [selectedTitle, setSelectedTitle] = useState("Flow Chart Data");
  ...
  const fetchTableData = async (page, rowsPerPage, period) => {
    let params = `type=flow&period=${period}`;
  ...
  const handleDownloadAllData = async () => {
    let params = `type=flow&period=${selectedPeriodForTable}`;

    if (selectedPeriodForTable === "daily" && selectedMonth && selectedYear) {
      params += `&month=${selectedMonth}&year=${selectedYear}`;
    } else if (selectedPeriodForTable === "monthly" && selectedYear) {
      params += `&year=${selectedYear}`;
    }

    setLoadingdw(true);
    try {
      const url = `http://localhost:9921/api/tableStatistics/all?${params}`;
      ...
      link.setAttribute(
        "download",
        `Statistics_Flow_${selectedPeriodForTable}.csv`
      );
  ...
  const fetchCardData = async () => {
    try {
      const [
        ...
        fetch(
          `http://localhost:9921/api/statistics?period=now&type=flow`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=day&type=flow`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=month&type=flow`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
        fetch(
          `http://localhost:9921/api/statistics?period=year&type=flow`,
          {
            credentials: "include",
          }
        ).then((res) => res.json()),
      ...
      const flowData = dataLimitBatas.find((item) => item.data === "flow");
      let batasAtas = flowData?.upperlimit ?? 90;
      let batasBawah = flowData?.bottomlimit ?? 20;
      ...
      setMinMaxAvgNow({
        minNow: dataNowResponse.min_flow,
        maxNow: dataNowResponse.max_flow,
        avgNow: dataNowResponse.avg_flow,
        stddevNow: dataNowResponse.stddev_flow,
        trendNow: dataNowResponse.trend_status,
        outOfLimitNow: dataNowResponse.out_of_limit_duration,

        minDaily: dataDaily.min_flow,
        maxDaily: dataDaily.max_flow,
        avgDaily: dataDaily.avg_flow,
        stddevDaily: dataDaily.stddev_flow,
        trendDaily: dataDaily.trend_status,
        outOfLimitDaily: dataDaily.out_of_limit_duration,

        minMonthly: dataMonthly.min_flow,
        maxMonthly: dataMonthly.max_flow,
        avgMonthly: dataMonthly.avg_flow,
        stddevMonthly: dataMonthly.stddev_flow,
        trendMonthly: dataMonthly.trend_status,
        outOfLimitMonthly: dataMonthly.out_of_limit_duration,

        minYearly: dataYearly.min_flow,
        maxYearly: dataYearly.max_flow,
        avgYearly: dataYearly.avg_flow,
        stddevYearly: dataYearly.stddev_flow,
  ...
  const handleCardClick = async (data) => {
    try {
      ...
      let params = `type=flow&period=${selectedPeriodForApi ?? "daily"}`;
      ...
      if (data.rule === "stddev") {
        setSelectedTitle(`Standard Deviation Flow`);
        setSelectedData(
          dataResponse.map((item) => ({
            timestamp: item.record_time,
  ...
  const fetchChartData = async (startDate, endDate, selectedType) => {
    try {
  ...
  return (
    <>
      <div className="bg-white rounded-lg p-4 mb-5 flex items-center justify-between mt-10 md:mt-20 lg:mt-5">
        <p className="text-2xl font-bold"> Statistics Flow </p>
      </div>
      ...
      <div>
        <div className="pt-3 flex-col">
          ...
          {selectedData && (
            <LineChartAnalytic chartData={selectedData} title={selectedTitle} />
          )}

...
const Register = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:9921/api/register",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ full_name: id, password, pin_number: pin }),
        }
      );
  ...
  const requestPin = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:9921/api/request-pin",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        setMessage(data.message || "OTP telah dikirim ke email admin.");
      } else {
      ...
      setIsRequestingPin(false); // Selesai loading
    }
  };
      ...
      <div className="form text-center md:w-[50%] my-auto mx-auto">
        ...
        <div className="flex justify-center items-center mt-6">
          <div className="flex items-center h-[80px] w-[80%] border-2 border-gray-300 rounded-[10px] relative">
            ...
            />
            <button
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-sky-950 text-white px-4 py-2 rounded-[5px] text-[14px] font-medium flex justify-center items-center"
              onClick={requestPin}
              disabled={isRequestingPin} // Disable button saat loading
            >
              {isRequestingPin ? (
